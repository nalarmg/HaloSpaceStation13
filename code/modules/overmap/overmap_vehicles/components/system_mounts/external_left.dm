...
/datum/vehicle_mount/fourbyfour/left
	name = "Left Wing (under)"
	name_abbr = "WING (L-U)"
	mount_type = MOUNT_EXT
	mount_size = MOUNT_LARGE

/datum/vehicle_mount/fourbyfour/left/small
	name = "Left Wing (over)"
	name_abbr = "WING (L)"
	mount_size = MOUNT_SMALL

/datum/vehicle_mount/fourbyfour/left/position_weapon(var/obj/structure/vehicle_component/weapon/weapon)
	...
	switch(oriented_dir)
		if(NORTH)
			weapon.proj_start = locate(my_vehicle.x, my_vehicle.y+1, my_vehicle.z)
		if(EAST)
		...
		if(WEST)
			weapon.proj_start = locate(my_vehicle.x+2, my_vehicle.y, my_vehicle.z)
