...
/datum/plot_vector/proc/setup_heading(var/turf/S, var/heading, var/xo = 0, var/yo = 0)
	source = S
	angle = heading
	...
	//this should approximately match the existing projectile speed... test it at some point
	/*var/maxval = max(abs(offset_x), abs(offset_y))
	offset_x /= maxval
	offset_y /= maxval*/
