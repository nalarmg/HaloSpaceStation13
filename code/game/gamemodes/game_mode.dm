...
/datum/game_mode/proc/send_intercept()
	...
	for(var/mob/living/carbon/human/man in player_list) if(man.client && man.mind)
		...
		if (special_role in disregard_roles)
			continue
		else if(man.client.prefs.UNSC_relation == COMPANY_OPPOSED && prob(50) || \
			man.client.prefs.UNSC_relation == COMPANY_SKEPTICAL && prob(20))
			suspects += man
		// Antags
...
proc/get_nt_opposed()
	...
	for(var/mob/living/carbon/human/man in player_list)
		if(man.client)
			if(man.client.prefs.UNSC_relation == COMPANY_OPPOSED)
				dudes += man
			else if(man.client.prefs.UNSC_relation == COMPANY_SKEPTICAL && prob(50))
				dudes += man
